...
def convert_shapefile(shapefilename, srid=4674):
    ...
    for f in ds[0]:
        ...
        if is_uf:
            kwargs['nome'] = capitalize_name(unicode(f.get('NM_ESTADO'), 'utf-8'))
            kwargs['geom'] = g.ewkt
            kwargs['id_ibge'] = f.get('CD_GEOCUF')
            kwargs['regiao'] = capitalize_name(unicode(f.get('NM_REGIAO'), 'utf-8'))
            kwargs['uf'] = UF_SIGLAS_DICT.get(kwargs['id_ibge'])
        else:
            kwargs['nome'] = capitalize_name(unicode(f.get('NM_MUNICIP'), 'utf-8'))
            kwargs['geom'] = g.ewkt
            kwargs['id_ibge'] = f.get('CD_GEOCMU')
            kwargs['uf'] = UF.objects.get(pk=f.get('CD_GEOCMU')[:2])
